...
- **Source of Truth:**
  - NEVER manually type a database interface. ALWAYS import: `import { type User } from "@/db/schema"`.
  - Use `typeof table.$inferSelect` for reads and `typeof table.$inferInsert` for writes.
- **The "Null vs Undefined" Rule:**
  - Drizzle returns `null` for empty fields; React/Zod expects `undefined` or default values.
  - **MANDATORY:** When passing DB data to a Client Component, you MUST sanitize it in the Server Action or Parent Component:
    `const safeData = { ...data, bio: data.bio ?? "" };`
- **Server Action Return Types:**
  - Standardize all action returns. Do not return raw data.
  - Use a generic wrapper: `Promise<{ success: boolean; data?: T; error?: string }>`
- **No "Trust Me" Code:**
  - Do not assume a property exists. Use optional chaining (`data?.user?.id`) aggressively.