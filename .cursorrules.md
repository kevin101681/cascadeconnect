...
- **Type-First Approach:** Before writing React components, ALWAYS define the data interface or fetch the Zod schema first.
...
- **Logging:** Log Vapi payloads with `console.log("🔹 Vapi:", ...)` for visibility.
...
## 6. TypeScript & Data Integrity (Strict Mode)
- **Zero `any` Policy:** Never use `any`. Use `unknown` if the type is truly dynamic, or define a generic.
- **Drizzle Type Inference:**
  - DO NOT manually type interfaces for DB tables.
  - ALWAYS derive types from the schema: `type User = typeof users.$inferSelect;`
  - This ensures TypeScript instantly breaks if the DB schema changes, catching errors early.
- **Server Action Validation (Zod):**
  - All Server Actions MUST start with Zod validation.
  - If a Server Action receives raw data (e.g., from a form), parse it with Zod *before* passing it to Drizzle.
- **Strict Null Checks:**
  - Assume all optional database fields can be `null`.
  - Use optional chaining (`user?.address?.zip`) or explicit guards (`if (!data) return ...`).
- **Pre-Commit Check:**
  - Before confirming a task is "Done", run `npx tsc --noEmit` in the terminal to verify no silent type errors exist.