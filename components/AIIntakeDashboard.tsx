...
import { Phone, MapPin, Clock, AlertCircle, CheckCircle, XCircle, Calendar, Building2, User, Mail, ExternalLink, Play, Download, Search, ChevronLeft, ChevronRight, Trash2, ChevronDown, ChevronUp, StickyNote, Globe } from 'lucide-react';
...
const AIIntakeDashboard: React.FC<AIIntakeDashboardProps> = ({ 
  ...
  // Apply search filter from URL hash (deep link from Dashboard)
  useEffect(() => {
    const hash = window.location.hash;
    const searchMatch = hash.match(/[?&]search=([^&]*)/);
    if (searchMatch) {
      const searchParam = decodeURIComponent(searchMatch[1]);
                  ...
                  <button
                    onClick={() => {
                      ...
                      useTaskStore.getState().openTasks(
                        actualSelectedCall.homeownerId || null,
                        contextLabel,
                        'call'
                      );
                    }}
